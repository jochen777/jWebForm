...
public class MyFormBuilder {
  ...
  private TypeBuilder[] getAllTypeBuilders(boolean addUpload) {
    ...
    els.add(Type.select("gender", "", new String[] {"m", "f"}, new String[] {"Male", "Female"})
        .withLabel("Gender"));
    els.add(Type.submit());
    els.add(Type.checkbox("chk", true).withCriteria(req).withLabel("chk-label")
    ...
    els.add(
        Type.number("nbr", 5).withCriteria(req).withLabel("nbr-label").withHelptext("nrb-help"));
    els.add(Type.password("pssword").withLabel("Password"));
    if (addUpload) {
      els.add(Type.upload("upld").withLabel("Upload"));
    }
    els.add(Type.radio("radio", "1", new String[] {"1", "2"}, new String[] {"yes", "no"})
        .withLabel("Radio"));
