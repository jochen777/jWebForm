...
public class FormSubmitTest {
  ...
  public void test_EmptySubmit() {
    ...
    assertTrue("request does not have submit info, so submitted should be false", !fr.isSubmitted());
    assertTrue("... should not be submitted", !fr.isSubmittedAndNotOk());
  }

  @Test
  public void test_SubmitOK() {
    Form form = buildForm();
    FormResult fr = form.run(new EnvBuilder().of(ExampleRequests.exampleSubmittedRequest(FIRSTNAME, "Testname")));
    assertTrue("Form should be submitted" , fr.isSubmitted());
    assertTrue("Form should be validated and ok" , fr.isSubmittedAndOk());
  }

  @Test
  public void test_SubmitNotOK() {
    Form form = buildForm();
    FormResult fr = form.run(new EnvBuilder().of(ExampleRequests.exampleSubmittedRequest(FIRSTNAME, "")));
    assertTrue("Form should be submitted" , fr.isSubmitted());
    assertTrue("Form should submitted, but not okay." , !fr.isSubmittedAndOk());
  }
